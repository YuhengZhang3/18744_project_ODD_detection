...
def eval_stage1(model, loader, device, max_batches=0):
    model.eval()
    ...
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            ...
            total_state += y_state.numel()
            correct_state += int((pred_state == y_state).sum().item())

            total_severity += y_severity.numel()
            correct_severity += int((pred_severity == y_severity).sum().item())

    state_acc = correct_state / total_state if total_state > 0 else 0.0
    severity_acc = correct_severity / total_severity if total_severity > 0 else 0.0
    coarse_score = 0.5 * state_acc + 0.5 * severity_acc
...
def eval_stage2(model, loader, device, max_batches=0):
    model.eval()
    ...
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            ...
            total += y_road.numel()
            correct += int((pred == y_road).sum().item())
    ...
    return {
        "road_acc": correct / total if total > 0 else 0.0,
        "state_acc": correct_state / total_state if total_state > 0 else 0.0,
        "severity_acc": correct_severity / total_severity if total_severity > 0 else 0.0,
    }
...
def freeze_for_stage(model, stage):
    ...
    if stage == "stage1":
        for head_name in ["road_state", "road_severity"]:
...
def build_train_val_sets(rscd_root, augment, road_val_ratio, seed):
    ...
    return road_train, road_val, train_indices
...
def main():
    ...
    road_train, road_val, train_indices = build_train_val_sets(
    ...
    if args.resume_ckpt:
        ...
        if not args.init_ckpt:
            raise ValueError("need --init_ckpt when not resuming")
        ckpt = torch.load(args.init_ckpt, map_location="cpu")
        state = ckpt.get("model", ckpt)
        model.load_state_dict(state, strict=False)

    opt_road = torch.optim.AdamW(
        collect_trainable_params(model),
        lr=args.lr_road,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            if args.stage == "stage1":
                ...
                loss = (
                    ...
                    + args.lambda_severity * loss_severity
                )
        ...
        if args.stage == "stage1":
            ...
            score = val_metrics["coarse_score"]
            print(
                "[epoch {}/{}] total_loss {:.4f} state_loss {:.4f} severity_loss {:.4f} state_acc {:.4f} severity_acc {:.4f} coarse_score {:.4f} lr_road {:.6f} time {:.1f}s".format(
                    epoch, args.epochs,
                    running_loss / max(running_steps, 1),
                    running_state / max(running_steps, 1),
                    running_severity / max(running_steps, 1),
                    val_metrics["state_acc"],
                    val_metrics["severity_acc"],
                    val_metrics["coarse_score"],
            ...
            val_metrics = eval_stage2(
                model=model,
                loader=val_loader,
                device=device,
                max_batches=args.eval_max_batches,
            )
            score = val_metrics["road_acc"]
            print(
                "[epoch {}/{}] total_loss {:.4f} road_loss {:.4f} state_loss {:.4f} severity_loss {:.4f} road_acc {:.4f} state_acc {:.4f} severity_acc {:.4f} lr_road {:.6f} time {:.1f}s".format(
                    epoch, args.epochs,
                    running_loss / max(running_steps, 1),
                    running_road / max(running_steps, 1),
                    running_state / max(running_steps, 1),
                    running_severity / max(running_steps, 1),
                    val_metrics["road_acc"],
                    val_metrics["state_acc"],
                    val_metrics["severity_acc"],
                    opt_road.param_groups[0]["lr"],
                    time.time() - t0,
