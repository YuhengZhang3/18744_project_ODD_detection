...
def load_json_data(subfolder, basename):
    """Safely attempts to load a JSON file from a specific pipeline output folder."""
...
def draw_hud(img, text_lines):
    """Draws a semi-transparent black box with white text for readability."""
    if not text_lines:
        return img
...
def main():
    # Safely grab all source images to determine our loop
    image_paths = []
    ...
    while True:
        ...
        # 1. Always load the original raw image first
        ...
        # 2. Try to load the Glare mask (.png) and apply it as an overlay
        ...
        if os.path.exists(glare_mask_path):
            ...
            if mask is not None:
                # Ensure mask matches image dimensions just in case
                if mask.shape[:2] != img.shape[:2]:
                    mask = cv2.resize(mask, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST)
                
                # Create a yellow overlay where the mask detects glare
                overlay = img.copy()
                overlay[mask >= 128] = [0, 255, 255] # Bright Yellow in BGR
                
                # Blend the overlay (30% opacity) into the original image
                cv2.addWeighted(img, 0.7, overlay, 0.3, 0, img)

        # 3. Resize for display if the image is massive
        max_display_height = 800
        if img.shape[0] > max_display_height:
            scale = max_display_height / img.shape[0]
            img = cv2.resize(img, (int(img.shape[1] * scale), int(img.shape[0] * scale)))

        # Load data from the remaining 3 pipeline stages
        weather_data = load_json_data("weather", basename)
        cloud_data = load_json_data("cloud_detection", basename)
        vis_data = load_json_data("visibility", basename)

        # Build the text lines for the HUD
        hud_lines = [f"FILE: {filename}", "-"*30]

        # Weather HUD
        if weather_data and "weather" in weather_data:
            w = weather_data["weather"]
            hud_lines.append(f"WEATHER: Fog({w.get('fog_severity', 0):.2f}) Rain({w.get('rain_severity', 0):.2f}) Snow({w.get('snow_severity', 0):.2f})")
        else:
            hud_lines.append("WEATHER: No Data")

        # Visibility HUD
        if vis_data and "visibility_score" in vis_data:
            score = vis_data["visibility_score"]
            near = vis_data.get("near_contrast", "N/A")
            far = vis_data.get("far_contrast", "N/A")
            hud_lines.append(f"VISIBILITY: Score {score} | Near {near:.3f} | Far {far:.3f}")
        else:
            hud_lines.append("VISIBILITY: No Data")
        ...
        # Glare HUD
        hud_lines.append(f"GLARE: See visual overlay")
        ...
        if key in [ord('q'), 27]: # 'q' or ESC
            break
        elif key in [ord('d'), 83, 100]: # 'd', 'D', or Right Arrow
            idx = (idx + 1) % len(image_paths)
        elif key in [ord('a'), 81, 97]: # 'a', 'A', or Left Arrow
            idx = (idx - 1) % len(image_paths)
