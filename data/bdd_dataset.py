...
class BDDDTimeScene(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- NEW: Dynamic Masking ---
        # Set mask to 0.0 if the label is undefined to ignore it during loss calculation
        t_mask = 0.0 if t_str == "undefined" else 1.0
        s_mask = 0.0 if s_str == "undefined" else 1.0

        # --- NEW: CUDA Crash Prevention ---
        # If undefined, safely force the label integer to 0 to prevent out-of-bounds 
        # CUDA asserts. (The loss mask of 0.0 guarantees this label is ignored anyway).
        if t_mask == 0.0:
            t_label = 0
        if s_mask == 0.0:
            s_label = 0
        ...
        mask = {
            "time": torch.tensor(t_mask, dtype=torch.float32),
            "scene": torch.tensor(s_mask, dtype=torch.float32),
        }
